...
const OrderScreen = () => {
    ...
    useEffect(() => {
        const getPayPalClientId = async () => {
            const { data: clientId } = await axios.get('https://techshop-gj1f.onrender.com/api/config/paypal');
        ...
        const fetchOrder = async () => {
            try {
                const { data } = await axios.get(`https://techshop-gj1f.onrender.com/api/orders/${orderId}`);
                setOrder(data);
                setLoading(false);
            } catch (error) {
    ...
    const successPaymentHandler = async (paymentResult) => {
        try {
            await axios.put(`https://techshop-gj1f.onrender.com/api/orders/${orderId}/pay`, paymentResult);
